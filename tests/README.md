...
Partially visible Hjemla and Off Market clicks now verify that neighbouring snapshots stay opaque, move outward in either direction, fully clear the viewport, and clean up their names/styles on Close. Intro line/opacity checks verify that the notes and media wait until the complete reveal ends.

The line entrance checks distinguish early and later line opacity, preserve paragraph size and complete text, restore plain text after entry, and hold notes/media until the final line finishes. Return checks verify 60% of the previous travel, a duration below 350 ms for a completed opening, and continuous reversal for an interrupted opening.
